...
$product_name  =$price =$shop = $description= $status = $img=  "";
$productname_err = $description_err = $price_err = $shop_err  = $status_err =$img_err= "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $input_name4 = trim($_POST["shop"]);
    if(empty($input_name4)){
        $shop_err = "Please Enter an Update a Product shop.";
    }elseif(!filter_var($input_name4, FILTER_VALIDATE_REGEXP, array("options"=>array("regexp"=>"/[0-9]/")))){
        $shop_err = "Please enter a valid Update Product Shop.";
    }else{
        $shop = $input_name4;
    }
    ...
    if(empty($productname_err) &&  empty($description_err) && empty($price_err)&& empty($shop_err) && empty($img_err) && empty($status_err)){
        // Prepare an update statement
        
        $sql="UPDATE products SET product_name='$product_name', description='$description' , price='$price' , shop='$shop' ,img='$img', status='$status'  WHERE product_id=$_GET[product_id];";
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <div class="form-group">
                            <label>Shop</label>
                            <input type="number" name="shop" class="form-control <?php echo (!empty($shop_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $shop; ?>">
                            <span class="invalid-feedback"><?php echo $shop_err ;?></span>
                        </div>
